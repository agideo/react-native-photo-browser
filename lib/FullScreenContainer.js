...
export default class FullScreenContainer extends React.Component {
  ...
  _renderRow(media: Object, sectionID: number, rowID: number) {
    ...
    return (
      <View key={`row_${rowID}`} style={styles.row}>
        <TouchableWithoutFeedback onPress={this._toggleControls}>
          <Photo
            ref={ref => this.photoRefs[rowID] = ref}
            lazyLoad
            zoom
            onZoomHorizontalOuterRangeOffset={this.onZoomHorizontalOuterRangeOffset}
  ...
  onZoomHorizontalOuterRangeOffset = (offset) => {
    let time = new Date().getTime();
    const timeInterval = time - this.lastHorizontalOuterRangeOffsetTime;
    let mediaListSize = this.props.mediaList.length;
    let index = this.state.currentIndex;

    if (this.lastHorizontalOuterRangeOffsetTime && timeInterval > 500 && timeInterval < 1300) {
      if (offset > 0 && index > 0) {
        this.openPage(index - 1, true);
      }

      if (offset < 0 && index < (mediaListSize - 1)) {
        this.openPage(index + 1, true);
      }
      this.lastHorizontalOuterRangeOffsetTime = null;
    }

    if (timeInterval > 1300 || !this.lastHorizontalOuterRangeOffsetTime) {
      this.lastHorizontalOuterRangeOffsetTime = time
    }
  }
